...
def build_reAct_prompt(question):
    ...
    Tho 1: 我需要获取李四的心电图数据
    Act 2: GetMatPath '李四'
    Obs 2: /home/ubuntu/aimodel/ecg/dev-A.json
    Tho 3: 我需要调用预测心律不齐函数
    Act 4: PredictEcg '/home/ubuntu/aimodel/ecg/dev-A.json'
    Obs 4: 该用户的心电图数据被分类为心房颤动,这是一种常见的心律失常，表现为心房的快速不规则跳动。心房颤动可能导致血液淤积并形成血栓,这有可能引起卒中。平均概率为 95.60%,最高概率为 97.30%,最低概率为 91.85% ...
    Tho 5: 李四的心电图数据预测结果为心房颤动,概率为95.60%, 这表示李四出现心律失常,表现为心房的快速不规则跳动。心房颤动可能导致血液淤积并形成血栓，这有可能引起卒中
    Act 6: Finish '根据李四的心电图数据，可预测为心房颤动,概率为95.60%, 这表示李四出现心律失常，表现为心房的快速不规则跳动。心房颤动可能导致血液淤积并形成血栓，这有可能引起卒中'
    {{ question }}
    """
...
def start_chain(userPrompt):
    prompt = build_reAct_prompt(userPrompt)
